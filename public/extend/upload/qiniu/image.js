...
$(function () {
    ...
    if (window.Dolphin && typeof Dolphin.uploader === 'object' && typeof Dolphin.uploader.register === 'function') {
        // 注册七牛云驱动
        Dolphin.uploader.register('qiniu:image', {
            /**
             * 在秒传检查失败后、实际上传开始前执行。
            ...
            async prepare(file, fileItem) {
                ...
                try {
                    ...
                    // 从后端获取上传票据（Token）
                    const response = await jQuery.post('/_uploader/qiniu/image/getToken.html', {file:fileInfo});
            ...
            async success(file, result, fileItem) {
                ...
                const domain = DolphinConfig?.upload?.['qiniu.image']?.domain;
                if (!domain) {
