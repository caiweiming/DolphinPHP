...
$(function () {
    ...
    if (window.Dolphin && typeof Dolphin.uploader === 'object' && typeof Dolphin.uploader.register === 'function') {
        ...
        // 注册阿里云驱动
        Dolphin.uploader.register('aliyun:image', {
            /**
             * 在文件哈希计算完成后，实际上传前执行。
            ...
            async prepare(file, fileItem) {
                try {
                    ...
                    // 从后端获取上传策略（Policy）
                    const policyResponse = await jQuery.post('/_uploader/aliyun/image/getPolicy.html', {file:fileInfo});
            ...
            async success(file, result, fileItem) {
                ...
                // 从全局配置获取域名
                const domain = DolphinConfig?.upload?.['aliyun.image']?.domain;
                if (!domain) {
