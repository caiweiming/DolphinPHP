-- CMS 应用升级 SQL 示例
-- 演示 upgrade.sql 在版本前进时从旧版生命周期演示应用平滑升级到最小示例应用
...
SET @cms_article_source_exists := (
  SELECT COUNT(*)
  FROM INFORMATION_SCHEMA.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'dp_cms_article'
    AND COLUMN_NAME = 'source'
);

SET @cms_article_source_sql := IF(
  @cms_article_source_exists = 0,
  'ALTER TABLE `dp_cms_article` ADD COLUMN `source` varchar(120) NOT NULL DEFAULT '''' COMMENT ''文章来源'' AFTER `cover`',
  'SELECT 1'
);

PREPARE cms_article_source_stmt FROM @cms_article_source_sql;
EXECUTE cms_article_source_stmt;
DEALLOCATE PREPARE cms_article_source_stmt;

INSERT INTO `dp_cms_category` (`name`, `slug`, `status`, `sort`, `remark`, `create_time`, `update_time`)
SELECT '产品动态', 'product-news', 1, 10, '升级脚本补齐的演示分类。', UNIX_TIMESTAMP(), UNIX_TIMESTAMP()
FROM DUAL
WHERE NOT EXISTS (
...
INSERT INTO `dp_cms_category` (`name`, `slug`, `status`, `sort`, `remark`, `create_time`, `update_time`)
SELECT '使用指南', 'guides', 1, 20, '升级脚本补齐的演示分类。', UNIX_TIMESTAMP(), UNIX_TIMESTAMP()
FROM DUAL
WHERE NOT EXISTS (
...
SELECT
  `id`,
  '欢迎升级到 CMS 最小示例应用',
  'welcome-upgrade-cms-demo',
  '这是一篇由 upgrade.sql 补齐的演示文章，用于说明旧版生命周期演示应用如何平滑升级到新版最小示例应用。',
  '当旧版 cms 只有 dp_cms_demo_lifecycle 表时，upgrade.sql 会自动创建分类表与文章表，并补齐最小演示数据，保证升级后可以直接访问后台 CRUD 与前台列表详情。',
  '',
  'upgrade.sql',
  1,
  10,
...
WHERE `slug` = 'guides'
  AND NOT EXISTS (
    SELECT 1 FROM `dp_cms_article` WHERE `slug` = 'welcome-upgrade-cms-demo'
  );

UPDATE `dp_cms_article`
SET `source` = 'upgrade.sql'
WHERE `source` = '';

INSERT INTO `dp_cms_demo_lifecycle` (`event_code`, `event_title`, `details`, `create_time`)
SELECT 'upgraded', '应用已升级', 'upgrade.sql 已执行：向 cms 生命周期演示表追加升级记录。', UNIX_TIMESTAMP()
FROM DUAL
WHERE NOT EXISTS (
  SELECT 1 FROM `dp_cms_demo_lifecycle` WHERE `event_code` = 'upgraded'
);
