...
namespace app\common\component\payment;
...
class Item implements Component
{
     ...
     */
    public function __construct(string $type = '', string $label = '', string $size = '', string|array $class = '')
    {
        $this->type($type)
            ->label($label)
            ->size($size)
    ...
    public function handle(array $params = []): string
    {
        if (!self::$templatePath) {
            self::$templatePath = dp_component_path() . 'payment' . DIRECTORY_SEPARATOR . 'item.html';
        }
    ...
    /**
     * 设置支付图标类型
     * @param string $type 支付图标类型
     * @return self
     */
    public function type(string $type): self
    {
        $this->params['type'] = $type ?: 'unionpay';
        return $this;
    }
